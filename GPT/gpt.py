...
class Head(nn.Module):
    def __init__(self, head_size) -> None:
        ...
        # 缓冲区是一种在模型中存储持久化状态的张量，
        # 它会被自动保存和加载，并且不会作为模型的可学习参数进行优化。
        # 缓冲区通常用于存储与模型状态相关的固定数据。
    ...
    def forward(self,x):
        ...
        # 行遮蔽操作，将注意力权重矩阵中对应下三角部分为零的位置（即未来位置）的值替换为负无穷大。
        # 这样做是为了确保模型在自注意力机制中只关注当前和过去的信息，避免关注未来位置的信息。
